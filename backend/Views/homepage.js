...
const get_homepage_posts = async (req, res) => {
	try {
		...
		pool.query(
            ...
            post_info_2 as (select post_info.post_id as post_id, post_info.poster_page_id as page_id, post_info.poster_user_id as user_id, post_info.content_type as content_type, post_info.content as content, post_info.time as time, post_rank, coalesce(count(reaction.reaction), 0) as reaction_count
            from (
